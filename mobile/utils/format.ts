...
/**
 * CareTip financial / numeric display — German/EU grouping & decimals to match web `formatEur`:
 * €20,00 · €1.240,50 · €15.240,00 · counts 1.240
 *
 * Symbol placement matches web (`€` prefix). Locale-aware copy (dates, labels) still uses
 * `uiLocaleTag()` elsewhere.
 */
const MONEY_LOCALE = "de-DE" as const;
...
/** Whole numbers use German grouping (1.240 — never 1,240). */
...
export function formatGrowthPercent(value: number | null | undefined): string {
  if (value == null || !Number.isFinite(value)) return "—";
  const sign = value > 0 ? "+" : "";
  const body = value.toLocaleString(MONEY_LOCALE, {
