...
function runStaticAudit() {
  ...
  if (stripeSvc.includes("refund_application_fee: true") && stripeSvc.includes("reverse_transfer: true")) {
    pass("AA-refund-app-fee", "Eligibility refund uses refund_application_fee + reverse_transfer");
    pass("AB-no-reverse-transfer", "Destination-charge full refund reverses the connected transfer (Stripe requires this when the application fee was taken on the transfer)");
